...
const STORAGE_SNAPSHOT_QUEUE = "storage-snapshot";
...
async function main(): Promise<void> {
  ...
  await boss.createQueue(STORAGE_SNAPSHOT_QUEUE);
  await boss.schedule(STORAGE_SNAPSHOT_QUEUE, "0 2 * * *");
  ...
  await boss.work(STORAGE_SNAPSHOT_QUEUE, async () => {
    await storageSnapshot();
  });

  console.log(
    `worker: listening on "${PROCESS_ASSET_QUEUE}", "${INGEST_USAGE_EVENTS_QUEUE}", "${DELIVER_WEBHOOK_QUEUE}", "${FETCH_URL_QUEUE}", "${PURGE_ASSET_QUEUE}", "${ROLLUP_USAGE_QUEUE}", "${STORAGE_SNAPSHOT_QUEUE}"`,
  );
}
